...
const updateProfile = async (req, res) => {
  try {
    ...
    if (!userId || !name || !phone || !dob || !gender || !address) {
      return res.json({ success: false, message: "Dataa Missing" });
    }
  ...
  try {
    ...
    if (slots_booked[slotDate]) {
      if (slots_booked[slotDate].includes(slotTime)) {
        return res.json({ success: false, message: "Slot not avaiable" });
      } else {
...
// Api to get user appointments for frontend my appointements page
...
const paymentRazorpay = async (req, res) => {
  try {
    ...
    const appointmentData = await appointmentModel.findById(appointmentId);
    if (!appointmentData || appoinmentData.cancelled) {
      return res.json({
        sucsess: false,
